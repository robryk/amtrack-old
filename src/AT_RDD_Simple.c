...
/* --------------------------------------------------- GEISS RDD ---------------------------------------------------*/
 ...
 double  AT_RDD_Geiss_Gy( const double r_m,
  ...
  if ((a0_m < r_m) && (r_m <= max_electron_range_m)){
    return norm_constant_Gy * square(a0_m / r_m);
  }
  return 0.;
}
...
/* --------------------------------------------------- KATZ RDD ---------------------------------------------------*/
 ...
 double AT_RDD_Katz_coeff_Gy( const double Z_eff,
  ...
  return C_J_m * 0.5 * M_1_PI * square(Z_eff/beta) / (density_kg_m3 * square(max_electron_range_m));
}
 ...
 double AT_RDD_Katz_PowerLawER_Dpoint_Gy(const double r_m,
  ...
  if( r_m > max_electron_range_m )
    ...
    return Katz_point_coeff_Gy * (1.0/alpha) * square(max_electron_range_m/r_m) * pow(1.0 - r_m/max_electron_range_m, 1.0 / (alpha));
}
...
/* --------------------------------------------------- CUCINOTTA RDD ---------------------------------------------------*/
 ...
 double   AT_RDD_Cucinotta_f_shortRange( const double r_m,
  ...
  return 1.0/(1e-9/r_m + 0.6 + 1.7 * beta + 1.1 * square(beta));
}
 ...
 double   AT_RDD_Cucinotta_f_longRange( const double r_m,
  ...
  return exp( - square( r_m / (0.37 * max_electron_range_m ) ) );
}
 ...
 double AT_RDD_Cucinotta_Ddelta_Gy( const double r_m,
  ...
  return Katz_point_coeff_Gy * AT_RDD_Cucinotta_f_longRange(r_m, max_electron_range_m) * AT_RDD_Cucinotta_f_shortRange(r_m, beta) * square(max_electron_range_m/r_m);
}
...
double   AT_RDD_Cucinotta_Dexc_Gy( const double r_m,
  ...
  return C_norm * Katz_point_coeff_Gy * exp( - 0.5 * r_m / d_m ) * square(max_electron_range_m/r_m);
}
