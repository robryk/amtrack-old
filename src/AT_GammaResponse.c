...
void AT_gamma_response( const long  number_of_doses,
  ...
  if(gamma_model == GR_Radioluminescence){
	  ...
	  for (i = 0; i < number_of_doses; i++){
		  if(d_Gy[i]  <= D1){
			  S[i]    =  c0 * d_Gy[i] + 0.5 * B * square(d_Gy[i]);
		  }else{
			  S[i]    =  c0 * D1 + 0.5 * B * square(D1) + Smax * (d_Gy[i] - D1);
		  }
	  }
	  if(lethal_events_mode){
		  for(i = 0; i < number_of_doses; i++){
  ...
  if(gamma_model == GR_LinQuad){
    ...
    for (i = 0; i < number_of_doses; i++){
		if( d_Gy[i] < D0 ){
			S[i]    =       alpha * d_Gy[i] + beta * square(d_Gy[i]);
		} else {
			S[i]    =       alpha * D0 + beta * square(D0) + ( alpha + 2.0 * beta * D0) * (d_Gy[i] - D0);
		}
  ...
  if(gamma_model == GR_Geiss){
	  ...
	  for (i = 0; i < number_of_doses; i++){
		  if(lethal_events_mode){
			  ...
			  S[i]    = -log(1.0 - k1 * (1.0 - exp(-a1 * d_Gy[i])) - k2 * (1.0 - exp(-a2 * square(d_Gy[i]))));
		  } else {
			  S[i]    = co * (k1 * (1.0 - exp(-a1 * d_Gy[i])) + k2 * (1.0 - exp(-a2 * square(d_Gy[i]))));
		  }
	  }
  }
