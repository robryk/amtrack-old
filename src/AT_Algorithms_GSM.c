...
void AT_GSM_sample_particle_positions(  const long  number_of_field_components,
        ...
        long i;
        double sample_grid_area_cm2 = square(100.0 * sample_grid_size_m);
    ...
    for (i = 0; i < nX; i++){        // x
      ...
      for (k = 0; k < number_of_field_components; k++){  // field components
        for (l = 0; l < number_of_particles_in_field_component[k]; l++){  // particles
          double r_m  =  sqrt( square(x_position[k][l] - cur_x_pos) + square(y_position[k][l] - cur_y_pos));
...
void AT_GSM_local_dose_distrib_from_dose_grid( const long     nX,
  ...
  for(i = 0; i < number_of_bins; i++){
    frequency[i]   /= square(nX);
  }
  (*zero_fraction) /= square(nX);
}
    ...
    double*        sd_n_particles)
{
  ...
  *sd_n_particles         = 0.0;
  ...
  for (k = 0; k < N_runs; k++){
    ...
    total_dose_on_grid_ions_Gy  /= square(nX);
    average_grid_response_ions  /= square(nX);
    ...
    *sd_relative_efficiency  += square(efficiency);
    *sd_d_check              += square(total_dose_on_grid_ions_Gy);
    *sd_S_HCP                += square(average_grid_response_ions);
    *sd_S_gamma              += square(gamma_response);
    *sd_n_particles          += square(n_particles);
  }/* End of main loop */
  ...
  *sd_relative_efficiency  -= square(*relative_efficiency);
  *sd_d_check              -= square(*d_check);
  *sd_S_HCP                -= square(*S_HCP);
  *sd_S_gamma              -= square(*S_gamma);
  *sd_n_particles          -= square(*average_n_particles);
