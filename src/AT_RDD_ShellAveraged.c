...
/* --------------------------------------------------- SHELL AVERAGE DOSE ---------------------------------------------------*/
 ...
 double   AT_RDD_Katz_LinearER_Daverage_Gy(  const double r1_m,
  ...
  return 2.0 * Katz_point_coeff_Gy * ( log(r2_m/r1_m) - (r2_m - r1_m)/max_electron_range_m ) / (square(r2_m/max_electron_range_m) - square(r1_m/max_electron_range_m));
}
...
double   AT_RDD_Katz_PowerLawER_DaverageKernel(  const double x1,
  ...
  return 2.0 * (C2 * HGF2 - C1 * HGF1) / (square(x2) - square(x1));
}
...
double   AT_RDD_Katz_PowerLawER_DaverageKernel_approx(  const double x1,
   ...
   const double F1 = (x1 / square(alpha)) * ( (x1 / (4.0 * alpha)) * (1.0/alpha - 1.0) - 1.0 ) + log(x1);
   const double F2 = (x2 / square(alpha)) * ( (x2 / (4.0 * alpha)) * (1.0/alpha - 1.0) - 1.0 ) + log(x2);
   return 2.0 * (F2 - F1) / (square(x2) - square(x1));
}
...
double   AT_RDD_Cucinotta_Ddelta_average_Gy(  const double r1_m,
  ...
  return (2.0 * Katz_point_coeff_Gy / (square(r2_m/max_electron_range_m) - square(r1_m/max_electron_range_m))) * delta_average_integral ;
}
...
double   AT_RDD_Cucinotta_Dexc_average_Gy(  const double r1_m,
  ...
  return (2.0 * Katz_point_coeff_Gy / (square(r2_m/max_electron_range_m) - square(r1_m/max_electron_range_m))) * (FB - FA) ;
}
...
double   AT_RDD_Cucinotta_Cnorm( const double r_min_m,
  ...
  const double LETfactor_Gy = LET_J_m * M_1_PI / (material_density_kg_m3 * (square(max_electron_range_m) - square(r_min_m)));
...
double   AT_RDD_Geiss_average_Gy(  const double r1_m,
  ...
  if( r2_m <= a0_m ){
    ...
    return (norm_Gy / (square(r2_m) - square(r1_m)) ) * ( square(a0_m) - square(r1_m) + 2.0 * square(a0_m) * log( r2_m / a0_m ));
  } else if ( r1_m >= a0_m ){
    return (norm_Gy / (square(r2_m) - square(r1_m)) ) * 2.0 * square(a0_m) * log( r2_m / r1_m );
  }
  return 0.0;
}
...
//      = rho * (pi rmax^2 - pi a0^2) * D_av(a0,rmax)
  return material_density_kg_m3 * M_PI * \
          (square(max_electron_range_m) - square(a0_m)) * \
...
double   AT_RDD_Katz_LinearER_DSite_Gy( const double r_m,
  ...
  if( r_m < a0_m ){
    return M_1_PI * (LET_J_m - dEdx_J_m)/ (material_density_kg_m3 * square(a0_m));
  } else {
