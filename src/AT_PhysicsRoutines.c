    ...
    double        gamma[])
{
 ...
 double AT_E_from_beta_single(  const double beta ){
  assert( beta < 1.0);
  return atomic_mass_unit_MeV_c2 * (sqrt(1.0 / (1.0 - square(beta))) - 1.0);
}
...
double AT_E_from_gamma_single( const double gamma ){
	double beta = sqrt(1.0-1.0/square(gamma));
    ...
    double	dsE2dz_MeV2_cm2_g[])
{
	...
	tmp                 				/=  4.0 * M_PI * square(e0_F_m);
	tmp                 				/=  square(MeV_to_J) * m_to_cm;
	for (i = 0; i < n; i++){
...
inline double AT_dose_Gy_from_fluence_cm2_single(  const double  E_MeV_u,
    const long    particle_no,
...
void AT_beam_par_physical_to_technical(  const long  n,
    const double * restrict fluence_cm2,
    const double * restrict sigma_cm,
    double * restrict N,
    double * restrict FWHM_mm)
{
  long  i;
  for (i = 0; i < n; i++){
      N[i]           = fluence_cm2[i] * sigma_cm[i] * sigma_cm[i] * 2.0 * M_PI;
    ...
    double sigma_cm[])
{
  ...
  for (i = 0; i < n; i++){
      ...
      fluence_cm2[i] = N[i] / ((sigma_cm[i] * sigma_cm[i]) * 2.0 * M_PI);
  }
}
    ...
    double         results_Gy[])
{
  ...
  double fluence;
  for( i = 0 ; i < n ; i++ ){
    fluence = (2.0/distance_m[i]) * (2.0/distance_m[i]) * M_1_PI * 1e-4;
 ...
 double AT_single_impact_fluence_cm2_single( const double E_MeV_u,
  ...
  return M_1_PI / square( max_electron_range_m * m_to_cm ) ; // pi * r_max_m^2 = Track area -> single_impact_fluence [1/cm2]
}
